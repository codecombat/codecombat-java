package com.codecombat.api.client.clans.exceptions;
...
@JsonDeserialize(
    using = UpsertClanException.Deserializer.class
)
public final class UpsertClanException extends Exception {
  private final Value value;

  private int statusCode;

  private UpsertClanException(Value value, int statusCode) {
    this.value = value;
  ...
  public static UpsertClanException other(Object unknownValue, int statusCode) {
    return new UpsertClanException(new UnknownErrorValue(unknownValue), statusCode);
  }
  ...
  private static final class UnknownErrorValue implements Value {
    ...
    public String toString() {
      return "UpsertClanException{" + "unknownValue: " + unknownValue + "}";
    }
  }

  static final class Deserializer extends JsonDeserializer<UpsertClanException> {
    @Override
    public UpsertClanException deserialize(JsonParser p, DeserializationContext ctx) throws
        IOException {
      Value value = ctx.readValue(p, Value.class);
      int statusCode = (int) ctx.getAttribute("statusCode");
      return new UpsertClanException(value, statusCode);
    }
  }
}
