...
@JsonDeserialize(
    using = GetMembersStatsException.Deserializer.class
)
public final class GetMembersStatsException extends Exception {
  private final Value value;

  private int statusCode;

  private GetMembersStatsException(Value value, int statusCode) {
    this.value = value;
  ...
  public static GetMembersStatsException other(Object unknownValue, int statusCode) {
    return new GetMembersStatsException(new UnknownErrorValue(unknownValue), statusCode);
  }
  ...
  private static final class UnknownErrorValue implements Value {
    ...
    public String toString() {
      return "GetMembersStatsException{" + "unknownValue: " + unknownValue + "}";
    }
  }

  static final class Deserializer extends JsonDeserializer<GetMembersStatsException> {
    @Override
    public GetMembersStatsException deserialize(JsonParser p, DeserializationContext ctx) throws
        IOException {
      Value value = ctx.readValue(p, Value.class);
      int statusCode = (int) ctx.getAttribute("statusCode");
      return new GetMembersStatsException(value, statusCode);
    }
  }
}
