...
@JsonDeserialize(
    using = GetLevelSessionException.Deserializer.class
)
public final class GetLevelSessionException extends Exception {
  private final Value value;

  private int statusCode;

  private GetLevelSessionException(Value value, int statusCode) {
    this.value = value;
  ...
  public static GetLevelSessionException other(Object unknownValue, int statusCode) {
    return new GetLevelSessionException(new UnknownErrorValue(unknownValue), statusCode);
  }
  ...
  private static final class UnknownErrorValue implements Value {
    ...
    public String toString() {
      return "GetLevelSessionException{" + "unknownValue: " + unknownValue + "}";
    }
  }

  static final class Deserializer extends JsonDeserializer<GetLevelSessionException> {
    @Override
    public GetLevelSessionException deserialize(JsonParser p, DeserializationContext ctx) throws
        IOException {
      Value value = ctx.readValue(p, Value.class);
      int statusCode = (int) ctx.getAttribute("statusCode");
      return new GetLevelSessionException(value, statusCode);
    }
  }
}
