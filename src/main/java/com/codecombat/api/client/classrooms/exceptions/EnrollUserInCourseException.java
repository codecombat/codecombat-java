...
@JsonDeserialize(
    using = EnrollUserInCourseException.Deserializer.class
)
public final class EnrollUserInCourseException extends Exception {
  private final Value value;

  private int statusCode;

  private EnrollUserInCourseException(Value value, int statusCode) {
    this.value = value;
  ...
  public static EnrollUserInCourseException other(Object unknownValue, int statusCode) {
    return new EnrollUserInCourseException(new UnknownErrorValue(unknownValue), statusCode);
  }
  ...
  private static final class UnknownErrorValue implements Value {
    ...
    public String toString() {
      return "EnrollUserInCourseException{" + "unknownValue: " + unknownValue + "}";
    }
  }

  static final class Deserializer extends JsonDeserializer<EnrollUserInCourseException> {
    @Override
    public EnrollUserInCourseException deserialize(JsonParser p, DeserializationContext ctx) throws
        IOException {
      Value value = ctx.readValue(p, Value.class);
      int statusCode = (int) ctx.getAttribute("statusCode");
      return new EnrollUserInCourseException(value, statusCode);
    }
  }
}
