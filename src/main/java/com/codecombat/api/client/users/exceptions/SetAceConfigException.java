...
@JsonDeserialize(
    using = SetAceConfigException.Deserializer.class
)
public final class SetAceConfigException extends Exception {
  private final Value value;

  private int statusCode;

  private SetAceConfigException(Value value, int statusCode) {
    this.value = value;
  ...
  public static SetAceConfigException other(Object unknownValue, int statusCode) {
    return new SetAceConfigException(new UnknownErrorValue(unknownValue), statusCode);
  }
  ...
  private static final class UnknownErrorValue implements Value {
    ...
    public String toString() {
      return "SetAceConfigException{" + "unknownValue: " + unknownValue + "}";
    }
  }

  static final class Deserializer extends JsonDeserializer<SetAceConfigException> {
    @Override
    public SetAceConfigException deserialize(JsonParser p, DeserializationContext ctx) throws
        IOException {
      Value value = ctx.readValue(p, Value.class);
      int statusCode = (int) ctx.getAttribute("statusCode");
      return new SetAceConfigException(value, statusCode);
    }
  }
}
