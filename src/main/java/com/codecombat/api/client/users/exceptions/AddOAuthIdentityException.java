...
@JsonDeserialize(
    using = AddOAuthIdentityException.Deserializer.class
)
public final class AddOAuthIdentityException extends Exception {
  private final Value value;

  private int statusCode;

  private AddOAuthIdentityException(Value value, int statusCode) {
    this.value = value;
  ...
  public static AddOAuthIdentityException other(Object unknownValue, int statusCode) {
    return new AddOAuthIdentityException(new UnknownErrorValue(unknownValue), statusCode);
  }
  ...
  private static final class UnknownErrorValue implements Value {
    ...
    public String toString() {
      return "AddOAuthIdentityException{" + "unknownValue: " + unknownValue + "}";
    }
  }

  static final class Deserializer extends JsonDeserializer<AddOAuthIdentityException> {
    @Override
    public AddOAuthIdentityException deserialize(JsonParser p, DeserializationContext ctx) throws
        IOException {
      Value value = ctx.readValue(p, Value.class);
      int statusCode = (int) ctx.getAttribute("statusCode");
      return new AddOAuthIdentityException(value, statusCode);
    }
  }
}
