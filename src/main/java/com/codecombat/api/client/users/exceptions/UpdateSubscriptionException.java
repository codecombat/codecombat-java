...
@JsonDeserialize(
    using = UpdateSubscriptionException.Deserializer.class
)
public final class UpdateSubscriptionException extends Exception {
  private final Value value;

  private int statusCode;

  private UpdateSubscriptionException(Value value, int statusCode) {
    this.value = value;
  ...
  public static UpdateSubscriptionException other(Object unknownValue, int statusCode) {
    return new UpdateSubscriptionException(new UnknownErrorValue(unknownValue), statusCode);
  }
  ...
  private static final class UnknownErrorValue implements Value {
    ...
    public String toString() {
      return "UpdateSubscriptionException{" + "unknownValue: " + unknownValue + "}";
    }
  }

  static final class Deserializer extends JsonDeserializer<UpdateSubscriptionException> {
    @Override
    public UpdateSubscriptionException deserialize(JsonParser p, DeserializationContext ctx) throws
        IOException {
      Value value = ctx.readValue(p, Value.class);
      int statusCode = (int) ctx.getAttribute("statusCode");
      return new UpdateSubscriptionException(value, statusCode);
    }
  }
}
