...
@JsonDeserialize(
    using = GetHeroException.Deserializer.class
)
public final class GetHeroException extends Exception {
  private final Value value;

  private int statusCode;

  private GetHeroException(Value value, int statusCode) {
    this.value = value;
  ...
  public static GetHeroException other(Object unknownValue, int statusCode) {
    return new GetHeroException(new UnknownErrorValue(unknownValue), statusCode);
  }
  ...
  private static final class UnknownErrorValue implements Value {
    ...
    public String toString() {
      return "GetHeroException{" + "unknownValue: " + unknownValue + "}";
    }
  }

  static final class Deserializer extends JsonDeserializer<GetHeroException> {
    @Override
    public GetHeroException deserialize(JsonParser p, DeserializationContext ctx) throws
        IOException {
      Value value = ctx.readValue(p, Value.class);
      int statusCode = (int) ctx.getAttribute("statusCode");
      return new GetHeroException(value, statusCode);
    }
  }
}
