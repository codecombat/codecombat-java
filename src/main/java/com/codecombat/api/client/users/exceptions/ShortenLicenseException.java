...
@JsonDeserialize(
    using = ShortenLicenseException.Deserializer.class
)
public final class ShortenLicenseException extends Exception {
  private final Value value;

  private int statusCode;

  private ShortenLicenseException(Value value, int statusCode) {
    this.value = value;
  ...
  public static ShortenLicenseException other(Object unknownValue, int statusCode) {
    return new ShortenLicenseException(new UnknownErrorValue(unknownValue), statusCode);
  }
  ...
  private static final class UnknownErrorValue implements Value {
    ...
    public String toString() {
      return "ShortenLicenseException{" + "unknownValue: " + unknownValue + "}";
    }
  }

  static final class Deserializer extends JsonDeserializer<ShortenLicenseException> {
    @Override
    public ShortenLicenseException deserialize(JsonParser p, DeserializationContext ctx) throws
        IOException {
      Value value = ctx.readValue(p, Value.class);
      int statusCode = (int) ctx.getAttribute("statusCode");
      return new ShortenLicenseException(value, statusCode);
    }
  }
}
