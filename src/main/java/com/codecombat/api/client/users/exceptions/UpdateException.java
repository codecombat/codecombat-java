...
@JsonDeserialize(
    using = UpdateException.Deserializer.class
)
public final class UpdateException extends Exception {
  ...
  private UpdateException(Value value, int statusCode) {
    this.value = value;
  ...
  public static UpdateException other(Object unknownValue, int statusCode) {
  ...
  private static final class UnknownErrorValue implements Value {
    ...
    public String toString() {
      return "UpdateException{" + "unknownValue: " + unknownValue + "}";
    }
  }

  static final class Deserializer extends JsonDeserializer<UpdateException> {
    @Override
    public UpdateException deserialize(JsonParser p, DeserializationContext ctx) throws
        IOException {
      Value value = ctx.readValue(p, Value.class);
      int statusCode = (int) ctx.getAttribute("statusCode");
      return new UpdateException(value, statusCode);
    }
  }
}
