...
@JsonDeserialize(
    using = FindUserException.Deserializer.class
)
public final class FindUserException extends Exception {
  private final Value value;

  private int statusCode;

  private FindUserException(Value value, int statusCode) {
    this.value = value;
  ...
  public static FindUserException other(Object unknownValue, int statusCode) {
    return new FindUserException(new UnknownErrorValue(unknownValue), statusCode);
  }
  ...
  private static final class UnknownErrorValue implements Value {
    ...
    public String toString() {
      return "FindUserException{" + "unknownValue: " + unknownValue + "}";
    }
  }

  static final class Deserializer extends JsonDeserializer<FindUserException> {
    @Override
    public FindUserException deserialize(JsonParser p, DeserializationContext ctx) throws
        IOException {
      Value value = ctx.readValue(p, Value.class);
      int statusCode = (int) ctx.getAttribute("statusCode");
      return new FindUserException(value, statusCode);
    }
  }
}
