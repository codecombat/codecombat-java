...
@JsonDeserialize(
    using = GetClassroomsException.Deserializer.class
)
public final class GetClassroomsException extends Exception {
  private final Value value;

  private int statusCode;

  private GetClassroomsException(Value value, int statusCode) {
    this.value = value;
  ...
  public static GetClassroomsException other(Object unknownValue, int statusCode) {
    return new GetClassroomsException(new UnknownErrorValue(unknownValue), statusCode);
  }
  ...
  private static final class UnknownErrorValue implements Value {
    ...
    public String toString() {
      return "GetClassroomsException{" + "unknownValue: " + unknownValue + "}";
    }
  }

  static final class Deserializer extends JsonDeserializer<GetClassroomsException> {
    @Override
    public GetClassroomsException deserialize(JsonParser p, DeserializationContext ctx) throws
        IOException {
      Value value = ctx.readValue(p, Value.class);
      int statusCode = (int) ctx.getAttribute("statusCode");
      return new GetClassroomsException(value, statusCode);
    }
  }
}
