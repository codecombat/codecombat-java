...
@JsonDeserialize(
    using = GrantLicenseException.Deserializer.class
)
public final class GrantLicenseException extends Exception {
  private final Value value;

  private int statusCode;

  private GrantLicenseException(Value value, int statusCode) {
    this.value = value;
  ...
  public static GrantLicenseException other(Object unknownValue, int statusCode) {
    return new GrantLicenseException(new UnknownErrorValue(unknownValue), statusCode);
  }
  ...
  private static final class UnknownErrorValue implements Value {
    ...
    public String toString() {
      return "GrantLicenseException{" + "unknownValue: " + unknownValue + "}";
    }
  }

  static final class Deserializer extends JsonDeserializer<GrantLicenseException> {
    @Override
    public GrantLicenseException deserialize(JsonParser p, DeserializationContext ctx) throws
        IOException {
      Value value = ctx.readValue(p, Value.class);
      int statusCode = (int) ctx.getAttribute("statusCode");
      return new GrantLicenseException(value, statusCode);
    }
  }
}
