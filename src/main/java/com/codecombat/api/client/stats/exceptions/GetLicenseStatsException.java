...
@JsonDeserialize(
    using = GetLicenseStatsException.Deserializer.class
)
public final class GetLicenseStatsException extends Exception {
  private final Value value;

  private int statusCode;

  private GetLicenseStatsException(Value value, int statusCode) {
    this.value = value;
  ...
  public static GetLicenseStatsException other(Object unknownValue, int statusCode) {
    return new GetLicenseStatsException(new UnknownErrorValue(unknownValue), statusCode);
  }
  ...
  private static final class UnknownErrorValue implements Value {
    ...
    public String toString() {
      return "GetLicenseStatsException{" + "unknownValue: " + unknownValue + "}";
    }
  }

  static final class Deserializer extends JsonDeserializer<GetLicenseStatsException> {
    @Override
    public GetLicenseStatsException deserialize(JsonParser p, DeserializationContext ctx) throws
        IOException {
      Value value = ctx.readValue(p, Value.class);
      int statusCode = (int) ctx.getAttribute("statusCode");
      return new GetLicenseStatsException(value, statusCode);
    }
  }
}
